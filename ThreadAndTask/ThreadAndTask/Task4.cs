...
namespace ThreadAndTask
{
    public class Task4
    {
        ...
        public void Run()
        {
            Console.WriteLine($"Task4.Run(), ThreadId={Thread.CurrentThread.ManagedThreadId}");
            _Resource1 = "共用資源初始化";
            State1 myPara1 = new State1 { _Value1 = "參數初始化" };

            Task<string>task1 = Task.Run<string>(() => myFunction1(myPara1)); // Task.Run() 是 Task.Factory.StartNew() 的簡化版, 可簡化呼叫的方式.
